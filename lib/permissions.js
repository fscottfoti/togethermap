...
userIdExpression = function (user) {
    if(!user) {
        // return a user that does not exist - passing empty dict
        // to find will match all documents
        return {creatorUID: 'DNE'};
    }
    var users = [user._id];
    var g = googleId(user);
    if(g)
...
isMine = function (o, user) {
    return o && o.creatorUID &&
        (o.creatorUID == user._id ||
...
isReadLoginreq = function (o) {
    if(!o)
        return false;
    return o.read_loginreq != true;
};
...
readPermission = function (user, cid) {
    ...
    if(!user) {
        return isReadPublic(c) || isReadLoginreq(c);
    }
