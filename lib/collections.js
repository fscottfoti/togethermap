...
Meteor.methods({
    ...
    removeCollection: function (_id) {
        var c = MCollections.findOne(_id);
        if(!writePermission(c, _id, getUser(this.userId), "collection")) {
            throw new Meteor.Error(403, "Permission denied to remove collection");
        }

        return MCollections.remove(_id);
    },
