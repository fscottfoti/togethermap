...
FactualConnector = {
    ...
    getAll: function () {
        ...
        if(Session.get('query_valid') != true) {
            // sometimes we don't want to do a query, probably because we're just
            // looking at a single place
            return;
        }
    ...
    getOne: function (key) {
        ...
        Meteor.call('factualGet', key, function (err, res) {
            ...
            Map.addShape(gj, obj.factual_id);
            FactualConnector.places.push(gj);
            Map.goToPlace(gj, false); // don't bounce
